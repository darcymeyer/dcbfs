...
@click.command()
@click.option(u'--file', '-f', 'f')
def upload_file_interface(f):
    '''
...
@click.command()
@click.option('--upload', '-u', 'action', flag_value='u')
@click.option('--examine', '-e', 'action', flag_value='e')
def open_shell(action):
    '''
    General interface for block storage system.
    '''
    if action.lower() == 'e':
        examine_block_interface()
    elif action.lower() == 'u':
        upload_file_interface()
    else:
        print('Not a defined action! Try again. ')
